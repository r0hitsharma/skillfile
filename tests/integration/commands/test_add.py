...
def test_add_triggers_install_when_targets_configured(tmp_path):
    ...
    with patch("skillfile.commands.add.sync_entry", return_value={}) as mock_sync:
        with patch("skillfile.commands.add.install_entry") as mock_install:
...
def test_add_no_install_when_no_targets(tmp_path, capsys):
    write_manifest(tmp_path)
    with patch("skillfile.commands.add.sync_entry") as mock_sync:
        cmd_add(_local("skill", "skills/new.md"), tmp_path)
...
def test_add_rollback_on_install_failure(tmp_path, capsys):
    ...
    with patch("skillfile.commands.add.sync_entry", side_effect=failing_sync):
